...
#define utt u32
#define stt s32
#define EMPTY CNUM8_EMPTY
#elif T == 16
#define ut2 u32
#define st2 s32
#define utt u32
#define stt s32
#define EMPTY CNUM16_EMPTY
#elif T == 32
#define ut2 u64
#define st2 s64
#define utt u32
...
bool FN(urange_overflow)(struct cnum_t cnum)
{
...
bool FN(srange_overflow)(struct cnum_t cnum)
{
	...
	if (FN(urange_overflow)(b1)) {
		if (b1.base <= a.size) {
			...
			return (struct cnum_t) {
				.base = a.base,
				.size = min(a.size, (ut)((utt)b1.base + b1.size)),
			};
		}
		...
		return (struct cnum_t) {
			.base = b.base,
			.size = min((ut)((utt)a.size - dbase), b.size),
		};
	} else {
		return EMPTY;
	}
...
bool FN(contains_cnum)(struct cnum_t a, struct cnum_t b)
{
	struct cnum_t b1;
...
struct cnum_t FN(complement)(struct cnum_t cnum)
{
	if (FN(is_empty)(cnum))
...
struct cnum_t FN(gap)(struct cnum_t a, struct cnum_t b)
{
	struct cnum_t b1;
	ut x;
...
struct cnum_t FN(extend)(struct cnum_t a, struct cnum_t b)
{
	struct cnum_t b1;
...
struct cnum_t FN(bigger)(struct cnum_t a, struct cnum_t b)
{
	if (FN(is_empty)(a))
		return b;
	if (FN(is_empty)(b))
...
static struct cnum_t FN(mk_mul_u)(utt a, utt b, utt c, utt d)
{
	...
	/*
	 * printf("mk_mul_u: a=%d, b=%d, c=%d, d=%d, size=%d\n",
	 *        (int)a, (int)b, (int)c, (int)d, (int)size);
	 */
	if (size >= UT_MAX)
		return (struct cnum_t){ 0, UT_MAX };
	return (struct cnum_t){ (ut)(a * b), (ut)size };
}

static struct cnum_t FN(mk_mul_s)(stt a, stt b, stt c, stt d)
{
	st2 size = (st2)((ut2)(st2)c * (ut2)(st2)d - (ut2)(st2)a * (ut2)(st2)b);

	/*
	 * printf("mk_mul_s: a=%d, b=%d, c=%d, d=%d, size=%d\n",
	 *        (int)a, (int)b, (int)c, (int)d, (int)size);
	 */
	if (size >= UT_MAX)
		return (struct cnum_t){ 0, UT_MAX };
	return (struct cnum_t){ (utt)a * (utt)b, size };
}
...
struct cnum_t FN(mul_chunk)(struct cnum_t a, struct cnum_t b)
{
	...
	/*
	 * printf("u={%d,%d} s={%d,%d}\n",
	 *        (int)u.base, (int)u.size, (int)s.base, (int)s.size);
	 */
