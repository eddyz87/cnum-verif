...
int main(int argc, char **argv)
{
	...
	if (argc == 6 && strcmp(argv[1], "--mul") == 0) {
		struct cnum64 a = { strtoul(argv[2], NULL, 0), strtoul(argv[3], NULL, 0) };
		struct cnum64 b = { strtoul(argv[4], NULL, 0), strtoul(argv[5], NULL, 0) };
		struct cnum64 r = cnum64_mul(a, b);

		printf("{%lx,%lx} * {%lx,%lx} = {%lx,%lx} u:[%lx..%lx] s:[%lx..%lx]\n",
		       a.base, a.size, b.base, b.size,
		       r.base, r.size,
		       cnum64_umin(r), cnum64_umax(r),
		       cnum64_smin(r), cnum64_smax(r));
		return 0;
	}

	if (argc == 4 && strcmp(argv[1], "--neg") == 0) {
		struct cnum8 a = { strtoul(argv[2], NULL, 0), strtoul(argv[3], NULL, 0) };
		struct cnum8 r = cnum8_negate(a);

		printf("-{%u,%u} = {%u,%u} u:[%u..%u] s:[%d..%d]\n",
		       a.base, a.size, r.base, r.size,
